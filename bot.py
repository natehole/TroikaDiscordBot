...
async def generateCharacter(ctx):
    try:
        ...
        skill_str = '**Skill - {}**\n\tFormula: 1d3 + 3\n\tRolled: ({})'.format(skill_total, skill_roll)

        stamina_r1 = roll(6)
        stamina_r2 = roll(6)
        stamina_total = stamina_r1 + stamina_r2  + 12
        stamina_str = '**Stamina - {}**\n\tFormula: 2d6 + 12\n\tRolled: ({}, {})'.format(stamina_total, stamina_r1, stamina_r2)
        
        luck_roll = roll(6)
        luck_total = luck_roll + 6
        luck_str = '**Luck - {}**\n\tFormula: 1d6 + 6\n\tRolled: ({}) + 6'.format(luck_total, luck_roll)

        pence_r1 = roll(6)
        pence_r2 = roll(6)
        pence_total =  pence_r1 + pence_r2
        pence_str = '**Silver Pence - {}**\n\tFormula: 2d6\n\tRolled: ({}, {})'.format(pence_total, pence_r1, pence_r2)

        bkg_r1 = roll(6)
        bkg_r2 = roll(6)
        bkg_str = '**Character Background - {}{}**\n\tFormula: 1d66\n\tRolled: ({}{})'.format(bkg_r1, bkg_r2, bkg_r1, bkg_r2)

        output_list = [
            '**Character Generated by {}!**'.format(ctx.author),
            skill_str,
            stamina_str,
            luck_str,
            pence_str,
